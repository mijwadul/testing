...
import { Plus, Edit, Trash2, UserCog, Crown, DollarSign, Users, HardHat } from 'lucide-react';
import { toast } from 'sonner';
import AlertModal from '../components/AlertModal';

// Bina-ERP Role Configuration (with legacy role support)
const ROLE_CONFIG = {
  gm: { label: 'General Manager', color: 'bg-purple-100 text-purple-800', icon: Crown, description: 'Full access + Final approval' },
  finance: { label: 'Finance Staff', color: 'bg-green-100 text-green-800', icon: DollarSign, description: 'Keuangan, Vendor, Payroll' },
  admin: { label: 'Admin/HR', color: 'bg-blue-100 text-blue-800', icon: Users, description: 'Equipment, Karyawan, Absensi' },
  field: { label: 'Field Staff', color: 'bg-amber-100 text-amber-800', icon: HardHat, description: 'Input lapangan: Absen, BBM, Work Logs' },
  // Legacy roles
  helper: { label: 'Helper (Legacy)', color: 'bg-gray-100 text-gray-800', icon: HardHat, description: 'Legacy - maps to Field Staff' },
  checker: { label: 'Checker (Legacy)', color: 'bg-gray-100 text-gray-800', icon: DollarSign, description: 'Legacy - maps to Finance Staff' }
};
  ...
  return (
    <div>
      ...
      <div className="bg-white rounded-lg shadow-sm p-4 mb-6">
        <h3 className="text-sm font-semibold text-gray-700 mb-3">Tingkat Akses Bina-ERP:</h3>
          ...
          <tbody className="bg-white divide-y divide-gray-200">
            {users.map((user) => (
              <tr key={user.id} className={user.id === currentUser?.id ? 'bg-blue-50' : ''}>
                ...
                <td className="px-6 py-4">{getRoleBadge(user.role || 'field')}</td>
                <td className="px-6 py-4">
